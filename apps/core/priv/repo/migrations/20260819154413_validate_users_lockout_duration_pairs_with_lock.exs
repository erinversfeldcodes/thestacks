defmodule Core.Repo.Migrations.ValidateUsersLockoutDurationPairsWithLock do
  @moduledoc """
      Validates the extended `users_login_lockout_state_consistent` added
      `NOT VALID` in the previous migration, so it covers the rows that were
      already there and not only the rows written from now on.
