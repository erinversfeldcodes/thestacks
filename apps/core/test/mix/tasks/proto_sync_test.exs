  ...
  describe "run/1 generate mode" do
    ...
    test "run([]) generates ecto schemas, dbt models, and migrations", %{tmp_dir: tmp_dir} do
      ...
      after
        ...
        |> File.ls!()
        |> Enum.filter(fn file ->
          # Remove ADD COLUMN drift migrations generated today, but keep CREATE TABLE
          # migrations (which may have been legitimately generated by this test run).
          String.starts_with?(file, today) and not String.contains?(file, "_create_")
        end)
        |> Enum.each(fn file ->
