...
defmodule Stacks.AI.ClientTest do
  ...
  describe "verify/3 — endpoint mapping" do
    test "maps verify to /verify" do
      req = Client.build_vision_request("/verify", %{})
      assert req.path == "/verify"
    end
  end

  describe "verify/3 — payload + HMAC signing" do
    # The /verify path is HMAC-signed identically to /analyze, /classify,
    # and /extract. We verify the signature shape rather than the network
    # round-trip — Modal needs a real GPU and never runs locally.
    test "produces a request body containing the three verify fields" do
      uploaded_image_url = "https://r2.example/uploads/abc"
      candidate_cover_url = "https://covers.openlibrary.org/b/isbn/9781476732123-L.jpg"
      candidate_isbn = "9781476732123"
      ...
      req = Client.build_vision_request("/verify", payload)
      decoded = Jason.decode!(req.body)
      assert decoded["uploaded_image_url"] == uploaded_image_url
      assert decoded["candidate_cover_url"] == candidate_cover_url
      assert decoded["candidate_isbn"] == candidate_isbn
    end

    test "X-Internal-Token header is present and structurally valid on /verify" do
      req =
        Client.build_vision_request("/verify", %{
          "uploaded_image_url" => "u",
          "candidate_cover_url" => "c",
          "candidate_isbn" => "9781476732123"
        })
  ...
  describe "verify/3 — mocked vision client" do
    defmodule MatchVerifyClient do
      @moduledoc false
      @behaviour Stacks.AI.ClientBehaviour
      @impl true
      def call_vision("verify", _payload),
        do: {:ok, %{"is_same_book" => true, "confidence" => 0.92, "reasoning" => "matches"}}

      def call_vision(_endpoint, _payload), do: {:ok, %{}}
    end

    defmodule MismatchVerifyClient do
      @moduledoc false
      @behaviour Stacks.AI.ClientBehaviour
      @impl true
      def call_vision("verify", _payload),
        do:
          {:ok, %{"is_same_book" => false, "confidence" => 0.4, "reasoning" => "different cover"}}

      def call_vision(_endpoint, _payload), do: {:ok, %{}}
    end

    defmodule VerifyErrorClient do
      @moduledoc false
      @behaviour Stacks.AI.ClientBehaviour
      @impl true
      def call_vision("verify", _payload), do: {:error, :service_unavailable}
      def call_vision(_endpoint, _payload), do: {:ok, %{}}
    end

    test "returns {:ok, %{is_same_book, confidence, reasoning}} on a verified match" do
      original = Application.get_env(:core, :vision_client)
      Application.put_env(:core, :vision_client, MatchVerifyClient)

      try do
        assert {:ok, %{"is_same_book" => true, "confidence" => 0.92, "reasoning" => _}} =
                 Client.verify(
                   "https://r2.example/uploads/abc",
                   "https://covers.openlibrary.org/b/isbn/9781476732123-L.jpg",
                   "9781476732123"
                 )
      after
        Application.put_env(:core, :vision_client, original)
    ...
    test "returns {:ok, %{is_same_book: false, ...}} on a mismatch" do
      original = Application.get_env(:core, :vision_client)
      Application.put_env(:core, :vision_client, MismatchVerifyClient)

      try do
        assert {:ok, %{"is_same_book" => false, "confidence" => _, "reasoning" => _}} =
                 Client.verify("u", "c", "9781476732123")
      after
        Application.put_env(:core, :vision_client, original)
      end
    end

    test "propagates upstream errors as {:error, reason}" do
      original = Application.get_env(:core, :vision_client)
      Application.put_env(:core, :vision_client, VerifyErrorClient)

      try do
        assert {:error, :service_unavailable} = Client.verify("u", "c", "9781476732123")
      after
        Application.put_env(:core, :vision_client, original)
      end
    end
