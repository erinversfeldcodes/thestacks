defmodule Stacks.ScraperModuleKeysTest do
  ...
  defp seeded_modules do
    ~r/scraper_module:\s*module/
    |> then(fn _ -> File.read!(@seeds) end)
    |> then(fn contents ->
      # The seed builds rows from a literal list of
      # {id, name, url, scraper_module, has_physical} tuples.
      Regex.scan(~r/\{\d+,\s*"[^"]+",\s*"[^"]+",\s*"([^"]+)",\s*(?:true|false)\}/, contents)
  ...
  test "the seed's scraper_module values are real registry keys, not basenames" do
    ...
    # basename but not its full key. A value matching nothing is a different thing —
    # a store whose config has not been written yet, which is expected and visible.
    mismatched =
      for m <- seeded,
          m not in keys,
