#!/usr/bin/env bash
# scripts/check-licenses.sh — check OSS licence compliance.
#
# Blocks GPL and AGPL licences in Elixir, frontend Node, and e2e Node
# dependency trees.
#
# Elixir: uses licensir (hex package). Install with:
#   add {:licensir, "~> 0.7", only: :dev, runtime: false} to mix.exs
# Node: uses license-checker via npx.
#
# Exit codes:
#   0 — all licences are acceptable (or tool unavailable)
#   1 — a GPL/AGPL licence was found
