...
- **Over-mocked integration:** an "integration" test where every boundary is stubbed — the wiring
  it claims to prove is exactly what it doesn't execute. (`BASE_URL` retargeting exists so real wiring is
...
2. MOCK TOPOLOGY — every mock, stub, fake, `Mox` expectation, `page.route()` interception, and
   mock-roster seam or `BASE_URL`-conditional path (file:line). For each: what real seam it replaces.
