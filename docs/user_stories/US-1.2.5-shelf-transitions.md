...
**Acceptance Criteria:**
- Horizontal slide for adjacent shelf navigation.
...
### Happy Path
1. User is on the Library shelf (`/library`).
...
6. The new shelf loads its data independently via `Api.getBookshelf "antilibrary"`.

### Room Transition Path
1. User is on the Library shelf.
...
### Sad Paths
- **Slow network**: The destination shelf renders its loading state (empty bookcase or "Loading...") during the transition. The transition animation is CSS-driven and does not wait for data.
...
No dedicated API call for transitions. Each destination shelf triggers its own `GET /api/bookshelves/:bookshelf_name` on init (see US-1.2.1 through US-1.2.4).

---

## 4. Auth & Middleware Guards

N/A -- transitions are purely client-side UI. Auth is checked independently by each destination shelf's API call.

---

## 5. Database Interactions

N/A -- transitions are CSS animations. Database queries are handled by the destination shelf's init.
...
| Any shelf | Reading Pile | Fade through darkness | Page type changes from `page--shelf` to `shelf-reading-pile` |
| Any shelf | Looking for Home | Fade through darkness | Page type changes |
| Reading Pile | Any shelf | Fade through darkness | Reverse room transition |
...
- **CSS classes**: Transition animation classes are driven by the `themeClass` field in each config: `shelf-library`, `shelf-antilibrary`, `shelf-wishlist`, `shelf-reading-pile`, `shelf-looking-for-home`. The wallpaper classes (`wallpaper--damask`, `wallpaper--botanical`, `wallpaper--floral`, `wallpaper--dragons`) also change, contributing to the parallax effect.
...
|--------|--------|------|-------------|-------------|
| `navigation.transition.count{from, to}` | Elm event tracking | Counter | Increment on each `UrlChanged` that triggers a shelf-to-shelf transition | Informational (navigation patterns) |
| `navigation.transition.type{type="slide"}` | Elm event tracking | Counter | Increment for adjacent-shelf horizontal slides | Informational |
...
Note: Transitions are purely client-side CSS animations. No HTTP requests or database queries are executed by the transition itself. The destination shelf's API call metrics are captured in the respective browse story (US-1.2.1 through US-1.2.4).
...
| `navigation.frequency_per_session` | Elm event tracking | Counter per session | Count of `UrlChanged` msgs that trigger shelf transitions | Informational (engagement) |
| `page.time_on_shelf{shelf}` | Elm event tracking | Histogram (s) | Time between entering a shelf route and navigating away | Informational (dwell time) |
...
| Neon DB (PostgreSQL) | N/A | N/A | No database queries during transitions. Destination shelf queries are accounted for in their respective browse stories. |
