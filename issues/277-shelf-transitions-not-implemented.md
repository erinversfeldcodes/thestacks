...
- [x] The transition class is cleared after the animation so repeat navigations re-trigger it — evidence: `277-repeat-retrigger.json`, four legs `/antilibrary → /library → /antilibrary → /library`, observed class sequence `[null, slide-in-right, null, slide-in-left, null, slide-in-right, null, slide-in-left, null]` — four distinct applications, each separated by a cleared state, each with a live `animation-name` and `0.3s`, ending clean
