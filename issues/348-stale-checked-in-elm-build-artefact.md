...
## Lead review 2026-07-31 — **LGTM**, with one finding corrected
**staff-review verdict: LGTM.** Praise: (a) it **re-derived the serving chain instead of trusting the memory note**, and found the decisive fact nobody had cited — `Plug.Static` at `endpoint.ex:17-21` has `only: ~w(assets textures favicon.ico robots.txt uploads)`, so `index.html` is not servable by the static pipeline at all and `PageController` is the only route to any index; (b) it proved the artefact was **never a build input** by hashing the three outputs, deleting the file, wiping `elm-stuff/` and the static dir, rebuilding clean, and getting **byte-identical** hashes — that is the right instrument for "was this load-bearing?"; (c) it identified what the file actually was (`elm make --output=index.html`, whole program inlined, no stylesheet link, no port wiring) and concluded it **could never have rendered the real app even when fresh**; (d) it found the one consumer — `package.json`'s `start` built `elm.js` then opened a file that ignores `elm.js` — and correctly read that as an argument *for* deletion rather than against.

**⚠️ Finding corrected by the lead.** The sibling survey reported 16 tracked files under `apps/core/priv/static/uploads/` as *"user-uploaded photographs in git, so GDPR-adjacent"*. **They are not.** All 16 contain the literal string `fake image content` — they are **test fixtures**, 64 KB in total, committed by accident. `file` reports `ASCII text` for every one. So the severity drops from a GDPR concern to minor hygiene, and the "history would need a separate rewrite" caveat does not apply.

The agent was right that they were tracked despite `.gitignore:99` claiming otherwise — the classic add-the-rule-after-the-fact trap this project has a convention about. Since nothing references any of the 16 UUIDs (verified: 0 hits across `apps/core/test`, `seeds.exs`, `e2e/`) and they are 18-byte text files, the lead **untracked them directly** rather than filing an issue for 64 KB: `git rm --cached`, files left on disk, `page_controller_test.exs` still 3/0. `git ls-files apps/core/priv/static/uploads/` → **0**, and `git check-ignore` now bites.

**Two doc-drift observations from the agent, left unactioned and worth someone's time:** `docs/user_stories/US-1.1.5-manual-isbn-entry.md` still documents the removed `lookupByIsbn` flow in the present tense (drift from #343, now the loudest remaining hit for the symbol), and `scripts/test-e2e.sh`'s header claims it serves the frontend on `:4001` when both it and `playwright.config.ts` use `:4000`.
Environment note also worth keeping: `frontend/public/textures/*.png` are git-lfs pointers in a fresh worktree, so textures are absent in a worktree drive — `just bootstrap-worktree` does not run `git lfs pull`.
