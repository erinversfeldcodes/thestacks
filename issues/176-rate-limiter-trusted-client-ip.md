...
- 2026-07-10: Implemented via orchestrator (security-agent). `get_ip/1` now keys on the trusted `fly-client-ip` header (fallback `conn.remote_ip`), never first XFF. TDD (RED→GREEN); testing-coordinator mutation-verified + flagged `:password_change` keying MISSING and the auth_controller migration WEAK → both closed (added `:password_change` spoof unit test + two-`fly-client-ip` `:api`-pipeline integration test). Gates: `just verify` exit 0 (2264 elixir tests); 52/0 in the two suites; 2B-iii deploy succeeded (Fly recovered) — 194 E2E passed through the deployed limiter, 1 orthogonal vision-test failure. elixir-reviewer APPROVED; PE GREEN (no P0/P1). Built on branch `176-…` off `feat/124-e2e-auth`.
- 2026-07-10: Scope expansion (human-directed, in-issue): also hardened `AuthController.get_ip/1` — the audit-log provenance IP path all three reviewers flagged with the identical XFF-trust weakness. Same fix as the RateLimiter (`fly-client-ip` → `conn.remote_ip`, never first XFF). Migrated the existing hashed-IP audit test to `Fly-Client-IP` + added an "audit IP does not trust X-Forwarded-For" test; mutation-verified (reverting flips both RED); auth_controller 33/0, rate_limiter 20/0; credo/sobelow/format clean. Vision E2E flake (`upload.spec.ts:352`) NOT filed per human (address later).
