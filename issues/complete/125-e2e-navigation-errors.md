...
- [x] No flaky tests — evidence: preview full chromium **281 passed / 0 failed** at DEFAULT workers + epic subset (settings/navigation/auth/age-gate) **62/0/0**, zero 429s (2026-07-26, `stacks-core-pr-feat-125-126-e2e.fly.dev`); local serial runs clean throughout. The documented `:auth`-bucket 4-worker artifact does not manifest on the gate environment — tracked as a local-dev-ergonomics follow-up in the epic state, not test nondeterminism.
- [x] `just verify` passes — evidence: the superset epic gate `just run just ci` green (run 2026-07-26): elixir/coveralls, elm, rust, dbt 239, squawk/checkov, licenses, gitleaks, trufflehog, Trivy, semgrep 0 findings; sole failure = dockle unable to reach the local Docker daemon (CLAUDE.md-documented local-only environment limitation; CI has Docker).
...
- [x] **Story docs reconciled**: `US-15.1.1` CTAs updated to shipped About/Marketplace; `US-16.3.1` "no global 401 handler" note updated to shipped interceptor behaviour — evidence: commit `5ff8d413` — `docs/user_stories/US-15.1.1-home-page.md` (+20/-6) and `docs/user_stories/US-16.3.1-unauth-redirect.md` (+19/-1) on this branch.
