...
2. Verify `proto/gen/elm/` is gitignored and regenerated at build time — the `--check` mode compares fresh generator output against the current `proto/gen/elm/` directory
3. Ensure `.gitignore` DOES ignore `proto/gen/elm/` (regenerated by `just setup`, `just dev`, and `scripts/deploy-stack.sh`)
...
- [ ] `proto/gen/elm/` is gitignored (regenerated at build time; CI uses `--check` mode)
