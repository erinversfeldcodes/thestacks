...
## Definition of Done
- [x] Door plays over the arrival, driven from `Arriving` — **LIVE-CONFIRMED 2026-08-06**: on a real owner login the dolly-shot played over the arrival (screenshot mid-animation) and the instrumented counter read `animationsStarted: 6` (>0 — the exact `=0` this issue fixed); door removed at `Authenticated`, reader stayed logged in. Original build note: `viewArrivalDoor : AuthState -> Html Msg` renders the scene (with the port's target ids `#bookshelf` etc.) from the shell over the destination on `Arriving _`, `text ""` on `Anonymous`/`Authenticated`. ⚠️ `animationsStarted > 0` **live drive owed at the epic coherence sweep** — not claimed from the code read (the code-read only shows the ids will be present for `getElementById`).
...
- [x] Frozen-animation counterfactual — the core guarantee (reader stays authenticated + navigated regardless of the animation) is proven two ways: the persist-first mutation-probe (removing `PersistAuth` reds `persist_first_no_animation_signal`) AND the live drive (door removed at `Authenticated`, still logged in). The CDP-freeze E2E spec is built in `auth.spec.ts`. Original: SPEC BUILT in `e2e/tests/auth.spec.ts` (`#364` block) reusing #359's CDP `Animation.setPlaybackRate: 0` + unresolvable-`finished` technique, asserting the stall took hold before asserting still-authenticated-and-navigated; live run owed at the coherence sweep.
