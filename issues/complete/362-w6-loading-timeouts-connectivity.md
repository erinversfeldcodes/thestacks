...
- [x] #359's persist-first probe still reddens — evidence: re-run 2026-08-01 on the integrated branch, removing `PersistAuth` from `Main.loginEffects`: `✗ persist_first_no_animation_signal` (`Nothing` vs `Just "jwt-token"`), `✗ persist_first_before_any_animation`, `✗ still persists the auth and navigates to the page they asked for` — `TEST RUN FAILED — Passed: 1544, Failed: 5`. Reverted with Edit; 1549/0, `git diff` empty.
- [x] Every behaviour has a validation path — evidence: `frontend/tests/Page/BookshelfProgramTest.elm "bookshelf_loading_is_not_the_empty_state"` (loading≠empty), `bash scripts/check-http-timeouts.sh` (bounding), `frontend/tests/ConnectivityTest.elm` 5 tests (banner); plus the epic wave drive 2026-08-01 for the two user-facing surfaces, where each shelf request was read off the live XHR as `timeoutMs: 15000` (`/api/bookshelves/{library,wishlist,reading_pile,antilibrary}`) and the offline banner + non-empty failed shelf were screenshot-captured.
...
- [x] **Test audit (embedded above) is GREEN** — evidence: 0 ❌ / 0 ⚠️ in the `## Test Audit` table above; every row cites a real test or gate (`frontend/tests/Page/BookshelfProgramTest.elm`, `frontend/tests/ConnectivityTest.elm`, `scripts/check-http-timeouts.sh`), re-verified green on the integrated branch by `npx elm-test` → `Passed: 1549, Failed: 0`.
- [x] `completion-audit` **passed 2026-08-01** on the integrated branch `feat/campaign-w6-316`. All six children driven live on the preview (screenshot-backed); #332's missing mutation probe run during the audit (`mutationToken` → `if False` reddened `read_only_synthetic_organiser_msg_SECURITY`, `expectHttpRequests: 0 vs 1`, 6 failures; reverted with Edit, 21/0 green, `git diff` empty); 19 evidence-free DoD boxes filled with cited artifacts; all 19 referenced issue numbers confirmed to have backing files; stale campaign-state corrected. `just run just ci` **green on every group** — elixir, elm (1549 tests), rust, python, proto, dbt (64 + 243 models), squawk, licences, and security (checkov 367/0, semgrep 516 rules/0 findings, trufflehog 0 secrets, trivy clean, dockle CIS pass).
- [x] **Meets the Completion Bar** — connectivity chain driven live in the epic's wave drive, 2026-08-01: offline banner rendered, shelf showed "The library is unreachable" **without repainting the bookcase as empty**, and the banner cleared on reconnect. Screenshots captured. ⚠️ The drive also found the banner's reconnect signal never reaches the shelf — filed as #368, not a regression of this issue. **done in the epic's wave drive, 2026-08-01** — full `--project=chromium` suite run against `stacks-core-pr-feat-campaign-w6-316.fly.dev`: **282 passed / 9 failed / 12 skipped, zero 502s** (`scratchpad/w6-e2e-1gb.log`). Every Wave 6 spec green. All 9 failures triaged to causes outside this wave and filed: #371 (3 = parallel MFA artefact, pass at `--workers=1`), #372 (4 = deterministic, pre-existing, not Wave 6), #370 (2 = the `barcode_unverified` data defect). ⚠️ The first run showed 23 failures; 14 were one OOM-restart loop, root-caused and filed as #369.
...
**staff-review verdict: LGTM** (2026-08-01, lead, Mode B on 5859eada). Praise: (a) **the census is a count, not a sample** — 37 modules, 56 `Loading ->` branches, every one examined two ways, and it reports **one genuine conflation in the whole app**. It could easily have claimed five (text-identical siblings) or fourteen (no loading affordance) and been "more impressive"; instead it read each and explained why thirteen of fourteen are correct by design — disabled `"Saving…"` buttons, booleans, `withDefault`. Honest scoping is worth more than a big number; (b) it found **the quieter half of the same bug**: `init` set `shelves = Loading` while issuing `Cmd.none` when there was no token — "a promise nothing would keep", which with a *real* loading state becomes a skeleton that shimmers for ever. State and command are now decided together, which is the actual fix; (c) **the test it replaced asserted `Selector.class "bookcase"`** — true in `Loading`, `Success []` and `Success [book]` alike, so it "passed throughout the defect's life". That is the precise shape this campaign keeps finding; (d) the timeout evidence is **real elapsed time against a stub that accepts the connection and never answers**: `send` → `timeout` event **15.002 s**, → skeleton 793 ms, → failure copy in the DOM **16 798 ms**, and the empty-bookcase state never present. Not a changed field asserted by a unit test; (e) the two timeout values are **argued, not picked** — 15 s sits above a cold Fly-machine start and inside human patience, and the single file-body request gets 120 s because its clock measures bytes moving rather than a server thinking, so cancelling it would make the timeout the bug; (f) **found only by looking**: the first skeleton row held 12 spines and filled half the bookcase, reading as an *emptying* shelf rather than a loading one. No test would ever have said so.
**Scope grew and it said so.** The sweep is **91 requests, not 80** — its new gate found 6 `Http.post`/`Http.get` shorthands with **no `timeout` field at all** (including `login` and `register`) plus 4 unbounded requests in pages. Leaving those would have reproduced the epic's own "correct decision, copied N times, no gate" complaint.
**Lead independent probe — the gate is not vacuous.** Reverted one `timeout = standardTimeout` to `Nothing`: `check-http-timeouts.sh` **exit 1**, naming `Api.elm:427` and the remedy, while `elm-test` reported **1483 passed, 0 failed**. `elm-program-test` never reads the field, so the gate covers a hop the suite structurally cannot. Reverted via Edit; `git status` clean; both this gate and #361's session-expiry gate back to exit 0.
**Prior guarantees verified intact after the merge:** session-expiry coverage exit 0 with its roster undiminished (24 pages / 68 endpoints); #359's persist-first probe still reddens 5; #332's `mutationToken` link and #361's `Authed`/`expectStringResponse` shapes untouched.
**⚠️ A harness discovery that explains earlier confusion in this campaign.** The driving tab reported `visibilityState: "hidden"` with **zero rAF callbacks in 3 s** — so Elm's DOM froze on a frame minutes old while the model advanced, and three "the timeout never fired" readings were that stale frame. This is **#359's founding condition recurring inside our own harness**. A screenshot forces a frame; a DOM/text read does not — so the two disagree and neither is trustworthy alone in a headless tab. This is the same effect that made `get_page_text` report a pre-click form twice during the Wave 4 and follow-ups drives. Recorded to project memory as a standing drive rule.
Suites: elm-test **1483/0** (baseline 1467, +16); `elm-review`, `elm-format`, `check-css.sh` 0 problems, `check-orphan-classes.sh` **88 / 0 unstyled** (baseline held), prose-assertions, e2e-vacuous-guards and admin-token gates all clean.
**Findings carried forward:** (1) **nothing ties an Elm port name to the `app.js` that answers it** — a typo makes `app.ports.X` undefined and the standard guard skips the block silently; #362 identified it as the only unprotected hop in the chain it had just built → filed as **#366**; (2) `Page/Bookshelf/ReadingPile.elm:297` renders its loading text in `reading-pile__empty-msg` — the *empty* state's class, i.e. the same conflation surviving at the CSS layer; (3) `Page/Profile.elm:64` has a bare `Loading…` where the shelf family now has a skeleton; (4) `ConnectivityChanged` deliberately triggers no refetch — whether reconnect should retry is a product decision for the owner.
