...
- [x] Feature-Completeness rows ✅ live (all four 🟡 surfaces built + driven); suites GREEN — Wave 8 code-gate `just ci` (2026-08-06): **elixir 3540/0, rust ok, dbt 367/0, security + squawk + licenses PASS** (exit 0); frontend elm **1743/0**, the 8 updated E2E specs **101/0** live vs the preview; orphans 0 / check-css clean. Deploy+E2E validated in the manual coherence drive; the full deploy+E2E gate re-runs at finalize-pr.
...
## Closed 2026-08-06 — Wave 8 complete
All eight phases (8a nav / 8b onboarding-D2 / 8c homes / 8d settings+consent-fold / 8e login-door / 8f a11y / 8g #389 / 8pre #387 specs) built, mutation-probed, gdpr-reviewed where data-touching, and staff-reviewed. Coherence sweep driven live 2026-08-06 (door animationsStarted:6, disclosures open/Escape/outside-click, collection-routing home, grouped settings + consent-fold + redirect, the two shelf-rooms, About) — the one finding (#390) fixed in-branch and re-driven, the onboarding focus-trap completed in-branch. Code gates green. Follow-ups all wave-assigned (#388→W11; the W11 cleanup group). Moving to `issues/complete/` under the cumulative-branch model; the single campaign PR merges at end of all waves.
