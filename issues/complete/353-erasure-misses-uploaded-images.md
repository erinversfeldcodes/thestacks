...
- [x] Verify-scope gates green — evidence: `just ci` run 2026-08-05 passed the full Elixir suite `3539 tests, 0 failures` on the same code (a later run showed 13 DBConnection ownership/timeout failures, proven environmental: they re-ran `246 tests, 0 failures` on a calm machine — the machine was at load-avg 36 from three back-to-back `just ci` runs, and every failure was a `DBConnection.ConnectionError`/`OwnershipError`/`ExUnit.TimeoutError`, never an assertion). dialyzer 0, credo 0, sobelow clean, dbt 367/0, squawk clean (incl. the two FK migrations), Trivy security PASS. The #353 GDPR suites specifically: `deletion_test.exs` + `gdpr_test.exs` = 39/0.
