...
Built in worktree; commit b67b4da8; merged into `feat/campaign-w5-315`. 2 files, +478/−118.
**staff-review verdict: LGTM** (2026-07-31, Mode B on b67b4da8). Praise: (a) it enumerated the field union **before** collapsing and asserted it by handing the *same facts to both entry points and comparing the rows column by column*, then added a `refute is_nil/1` loop so two all-null rows cannot satisfy the comparison vacuously — the anti-vacuity discipline applied without being asked; (b) it found a **second cost the issue did not state**: `verification_source_from/1` reads provenance off exactly the identifiers being dropped, so a collapse that failed to restore `google_books_id` would have silently downgraded every Google-Books-resolved confirm to `barcode_unverified` — the dropped field was not just a missing cross-reference but a false provenance claim; (c) the `merge_edition/2` conflict rule is justified on **security** grounds rather than taste: `BookController.merge_format/2` hands `merge_edition/2` the raw request params (`book_controller.ex:121`), so "caller wins on every field" would make `POST /api/books/:id/merge-format` a mass-assignment hole over the provenance columns; (d) probe 5 surfaced a race **in one of its own new tests** — `ISBNResolver.resolve/1` races OL and GB, and both were registered while asserting OL provenance, so the source was a coin flip; split into two single-source tests and verified under two seeds. Self-caught, reported, fixed.
**Lead independent probe (different target from the child's five):** I attacked the mass-assignment claim, since it is a security property the child introduced unprompted. Tests only ever pass `isbn`/`format_label`, so I expected the property to be reasoned-but-unguarded and probed it by making `insert_edition/5` prefer a caller-supplied `google_books_id`. **The probe does not compile** — `error: undefined variable "attrs"` — because the raw params map is never passed down: `merge_edition/2` extracts the two caller fields and forwards only those. The property is enforced by the function signature, which is **stronger than a test would be** (rung 3 rather than rung 1). My "unguarded" suspicion was wrong and the design is better than the issue asked for. Reverted via Edit; `git status` clean, `grep -c` → 1.
**Structural verification:** exactly **one** `Multi.insert(:book, …)` remains in the module; `create_confirmed_book/4` survives only as a thin adapter that translates atom-keyed resolver metadata and delegates; `build_book_attrs/3` and `maybe_create_author/2` are gone.
**Behaviour change flagged for the reviewer, correctly:** `create/1` used to swallow an author-insert failure and mint an author-less book; it now propagates `{:error, changeset}`, matching what the confirmed path already did and what `@spec` already allowed.
Suites: `books_test.exs` **86/0** (9 added), full core **3302/0** (15 properties, 9 excluded), credo clean, `proto.sync --check` clean.
**Finding carried forward → filed as #346:** `Moderation.build_book_attrs/4` (`moderation.ex:495-527`) drops `open_library_id` and `google_books_id` — the same defect class one caller upstream, so every edition minted from an upload is missing identifiers the resolver returned.
