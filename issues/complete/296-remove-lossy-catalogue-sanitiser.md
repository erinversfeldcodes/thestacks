...
- [x] No char-strip sanitiser remains in books.ex — all three are now gone: `search_books/2` (#291, commit 0d15326e), `maybe_search/2` (this issue), and the third at the old `books.ex:809` inside the DEAD `search_platform/2`, which was deleted wholesale under **#286** (owner Fork A: REMOVE). Evidence: `grep -n '~r/\[\^' apps/core/lib/stacks/books.ex` now matches only two explanatory comment lines, zero code sanitisers; `grep -rn search_platform apps/core/lib apps/core/test` → NONE.
