...
**staff-review verdict: LGTM** (2026-08-01, lead, Mode B on 896242ae). This is the best gate built in the campaign. Praise: (a) the `Authed` type forces `onExpired` through a **record literal**, and the agent named why that is stronger than the obvious alternative — an exhaustiveness check is satisfied by `_ ->`, a record literal is not. The wrong thing does not compile rather than merely failing review; (b) `authedExpect` is built on `expectStringResponse`, so **the 401 is claimed before the endpoint's resolver runs** — session expiry can no longer arrive disguised as `ProfileRequestFailed (BadStatus 401)`, which is the precise mechanism by which the "try again" lie reached the reader; (c) the reflection gate **discovers rather than lists**: a set difference recomputed every run over 68 mandatorily-authed endpoints → 24 pages, with **no page named in the discovery logic** (lead-verified: the only two file references in the script are a doc comment and a help message); (d) it correctly **excludes optional-auth endpoints** (`getProfile`, `getListings`, `getBlogPosts`, `searchUsers`) because those are valid anonymously, so their 401 is not an expiry signal — a gate that flagged them would be noisy and get disabled; (e) the single exemption is an **exemption map, not a roster** — it can only remove a named existing file, never leave a new page unchecked — and it prints its written reason on every run; (f) four escalating checks, each probed separately: undeclared → unexposed → declared-but-never-returned → wired to the wrong constructor.
**Lead independent probe — the crux, re-run rather than taken on trust.** Reintroduced the original defect verbatim in the *real* conversion: `Page/Settings/Password.elm`'s `onExpired = SessionExpiryDetected` → `SaveCompleted (Ok ())`, i.e. the 401 detected and then dropped. Result:
```
GATE: exit 1 — hands `Api.authed` an `onExpired` the page does not route to `SessionExpired`:
      SaveCompleted. The 401 is detected and then dropped.
elm-test: 1427 passed, 0 failed
```
**The gate catches what 1,427 Elm tests cannot** — the `Cmd` carrying the handler is opaque, so no Elm test can reach that link. That is the whole justification for a script gate over a unit test, demonstrated rather than argued. Reverted via Edit; `git status` clean, `grep -c` → 1, gate back to exit 0.
**Coordination held.** `Main.elm` stayed within the three additive blocks scoped in the 2026-08-01 decision, plus the extracted `redirectAfterNavigation` and its one call site — nothing else. The `redirectAfterLogin`-on-expiry finding from #359 was **closed here rather than deferred**: an expiry bounce now asks `loginRedirectFor` about the route being *left*, not `/login`, with a probe that reddens exactly the 2 behaviour tests and leaves all 3 controls green.
It also **paired its new `hasNot` with an explicit positive control** asserting the same string *does* render on a retryable failure — because `check-prose-assertions.sh` misses `ensureViewHasNot` (#356) and could not have caught a vacuous one.
Suites: elm-test **1427/0**, `elm-review` no errors, `elm-format --validate` `[]`, orphan classes **88 — zero added**, CSS 0 problems, full `lint-elm.sh` green.
**Findings carried forward:** (1) **63 of the 68 mandatorily-authed endpoints still take a bare `String` token** — protected by the gate, not by the type. Mechanical to convert but well past this issue's budget, and `Api.elm` goes to #362 next; worth its own issue so the type eventually covers what the gate currently watches. (2) `SessionExpiryPagesTest.elm` remains a hand-picked roster (8 → 11 pages); it is no longer the coverage claim, but the two should be reconciled so nobody reads the shorter list as the contract.
