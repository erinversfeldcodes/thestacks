...
- [x] Migrations applied cleanly on preview (Neon branch) + `just run just ci` green (squawk) — evidence: `PASS deploy: migrations applied`, `PASS deploy: migration integrity verified (105 repo migrations all applied)`; all three new constraints `convalidated = true` on `preview/feat-campaign-w4-314` (`book_editions_isbn_ean13_checksum`, `auth_token_families_user_id_fkey`, `guardian_tokens_user_id_fkey`). squawk PASS and **non-vacuous** — it linted 8 of the 9 new migrations once committed (see #337 for why it would otherwise have been vacuous).
