...
**staff-review verdict: LGTM** (2026-07-31, lead, Mode B on 0575f630). Praise: (a) the policy lives in **one place** — `scripts/generated-file-class.sh` — consulted by all three drift checks, and only *after* drift is already detected, so the clean path costs nothing; (b) **`untracked` fails closed**: an artefact that cannot be proven disposable is treated like a tracked one, which is the right default for a check whose whole job is deciding what may be silently rewritten; (c) it covered **all five** targets rather than the four in `lint-proto.sh`, on the grounds that `mix proto.sync --check` has the identical defect for the gitignored `gen/` tree and fixing four of five would leave `just verify` still crying wolf; (d) both counterfactuals use **the same file with the same drift, varying only tracked-ness** — that is the controlled experiment the claim needs, not two loosely related scenarios; (e) it chose `just regen-proto` **and** made `bootstrap-worktree`'s main-checkout branch `exec` it instead of exiting with advice, reasoning that "both incidents were failures of knowing" and that the command agents are already told to run first should simply be correct in both locations.
**A live bug found in passing:** `proto-sync-all` ran **four of the five** generators, omitting `gen-python-proto.sh` — precisely the artefact (`apps/vision/app/proto/gen/vision.py`) that Wave 5 tripped over. Anyone following the documented "regenerate everything" recipe would still have been left with a stale Python target. Verified fixed: `regen-proto.sh` now names the Python generator.
**Lead independent probe — the subtlest claim, which the child's own counterfactuals did not isolate.** The classifier asks `git ls-files --error-unmatch` *before* `git check-ignore`, because `check-ignore` is a pure pattern match and would call a tracked file ignored. I tested exactly that collision using a path covered by `.gitignore:99`: untracked → **`ignored`**; then `git add -f` the same file → **`tracked`**, while still matching the same pattern. Ordering is correct and load-bearing — reversed, a tracked generated file under an ignore pattern would be silently rewritten by a gate meant to fail on it. Probe removed; `git status` clean.
**Findings carried forward:** (1) ⚠️ **`scripts/ci.sh:92` runs `mix proto.sync` (generate, not `--check`) before any check** — the exact anti-pattern this issue's Reviewer Context warns against, already present and *pre-existing*: a **tracked** dbt model or `schema.yml` that genuinely drifted is silently rewritten inside the `elixir` group, and `just ci` never runs `proto-sync-check` at all, so that gate is effectively local-only. Not created or worsened here, but a real hole in the same family — worth its own issue. (2) `gen-elm-proto.sh`'s mtime shortcut is a **false signal** — it compares the *oldest* generated file's mtime against the sources, so a stale artefact with a fresh mtime reads as current; it printed "up to date" for a file the agent had just drifted (matches the project's standing "mtime staleness is a false signal" note — compare hashes, not timestamps).
**Honesty noted:** it disclosed damaging the `justfile` mid-task and restoring it, and generalised the cause into a reusable warning — *a test whose assertion is "the code must not write here" hands a real file to broken code*. Lead verified the `justfile` independently: no residue, parses, and the diff is exactly the intended `regen-proto` target plus `proto-sync-all` delegating to it.
Probes: Python branch → `if True` fails 3 tests; Elixir branch → always-regenerate fails 5. Suites: `proto_sync_test.exs` 103/0 (5 new), python 145 passed (11 new), shellcheck clean, credo clean.
