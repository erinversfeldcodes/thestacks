...
- [x] US-1.2.5 driven live: adjacent-shelf transition class captured **with screenshot** — evidence: `e2e/artifacts/270/125-b-during-adjacent-transition.png` + `125-adjacent.json` → `"mainClass":"app__main fade-through-dark-in"`, `"animationName":"none"`, `"animationDuration":"0s"`
- [x] US-1.2.5 driven live: room-change transition class captured **with screenshot** — evidence: `e2e/artifacts/270/125-e-during-room-transition.png` + `125-room.json` → `"mainClass":"app__main fade-through-dark-in"`, `"animationName":"none"`, `"animationDuration":"0s"` (identical to the adjacent case)
- [x] Nav stability across both transitions — evidence: `270-followup.json` → `.app-header` `{"position":"relative","top":0,"height":71}` **before and after** the transition. Note: the DoD's original `position: fixed` expectation is **wrong** — `main.css:172-173` sets `.app-header { position: relative }` by design. Recorded as geometric stability instead; see #277 Reviewer Context.
- [x] US-1.2.1 driven live via `aria-label` assertion, with row/lighting/sides/inner/width measurements — evidence: `e2e/artifacts/270/121-a-library-shelf.png` + `121-library.json` → `aria-label "Library"` (innerText `"LIBRARY"`), 4 `.shelf-row`, 1 `.lighting`, 2 `.bookcase__side`, 1 `.bookcase__inner`, 5 `.book-button`, `clientWidth == scrollWidth == 1012` (no overflow)
...
**Artifacts:** `e2e/artifacts/270/` — 9 screenshots + 5 JSON captures.
